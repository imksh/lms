...
const TopicsSection = ({ selectedSubject, subjects }) => {
  ...
  const saveTopic = async () => {
    ...
    try {
      ...
      if (!payload.subjectKey && selectedSubject)
        payload.subjectKey = selectedSubject.key;

      if (!isNew && !topicForm._id) {
        toast.error(
          "Internal Error: Topic ID is missing. Please refresh the page.",
        );
        setSaving(false);
        return;
      }
      ...
      const { data } = isNew
        ...
        : await cmsService.updateTopic(topicForm._id, payload);
      setSaveStatus("Saved!");
      fetchTopics(subjectKey);
  ...
  const setTask = (i, field, val) =>
    mutSec((s) => {
      s[i] = { ...s[i], task: { ...s[i].task, [field]: val } };
    });
  ...
  const addQuiz = () => {
    ...
    setNewQuiz({ question: "", options: ["", ""], correctAnswerIndex: 0 });
  };
  const removeQuiz = (i) =>
    setTopicForm((p) => ({
      ...p,
      quiz: (p.quiz || []).filter((_, qi) => qi !== i),
    }));
      ...
      <div className="w-72 shrink-0 border-r border-base-300/60 flex flex-col bg-base-200/40">
        ...
        {!selectedSubject && subjects.length > 0 && (
          <div className="p-3 border-b border-base-300/60">
            <select
              className="select select-xs select-bordered bg-base-100 rounded-xl text-xs w-full"
              value={topicForm.subjectKey}
              onChange={(e) => {
                setTopicForm((p) => ({ ...p, subjectKey: e.target.value }));
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {topics.map((t) => (
            <button
              ...
              className={`w-full text-left px-3 py-2.5 rounded-xl text-xs transition-all flex items-center justify-between group ${selectedTopicId === t.topicId ? "bg-primary text-primary-content font-bold shadow-sm" : "hover:bg-base-300/70 text-base-content/70 font-medium"}`}
            >
              ...
              <button
                ...
                className={`opacity-0 group-hover:opacity-100 p-0.5 rounded ${selectedTopicId === t.topicId ? "hover:bg-error/20 text-error" : "hover:bg-error/20 text-error/60"}`}
              >
                <Trash2 size={11} />
              </button>
            </button>
            ...
            {activeTab === "sections" && (
              <div className="flex flex-col gap-4">
                {tf.sections.map((sec, i) => (
                  <div
                    key={i}
                    className="card bg-base-200/60 border border-base-300/60 rounded-3xl p-5 flex flex-col gap-4"
                  >
                ...
                <button
                  onClick={addSec}
                  className="btn btn-sm btn-ghost rounded-2xl border border-dashed border-base-300 font-bold flex gap-1 items-center hover:border-primary/50 hover:text-primary"
                >
            ...
            {activeTab === "quiz" && (
              <div className="flex flex-col gap-4">
                {tf.quiz.map((q, i) => (
                  <div
                    key={i}
                    className="card bg-base-200/60 border border-base-300/60 rounded-2xl p-4 flex flex-col gap-2"
                  >
                    <div className="flex items-center justify-between">
                ...
                <div className="card bg-base-200/60 border border-base-300/60 rounded-3xl p-5 flex flex-col gap-3">
                  ...
                  <div className="flex flex-col gap-2 mt-1">
                      ...
                      <div key={i} className="flex items-center gap-1.5">
                        ...
                        {newQuiz.options.length > 2 && (
                          <button
                            ...
                            className="btn btn-xs btn-ghost text-error btn-square rounded-lg shrink-0"
                          >
                            <Trash2 size={14} />
                          </button>
                        )}
                    ...
                    {newQuiz.options.length < 4 && (
                      <button
                        ...
                        className="btn btn-xs btn-outline border-base-300 text-base-content/60 self-start rounded-lg mt-1"
                      >
                  ...
                  <button
                    type="button"
                    onClick={addQuiz}
                    className="btn btn-sm btn-primary rounded-xl font-bold self-start flex gap-1 items-center mt-2 disabled:opacity-50"
                    disabled={
